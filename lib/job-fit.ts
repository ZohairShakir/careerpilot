...
export const jobFitSchema = {
  ...
  properties: {
    ...
    summary: { type: "string" },
    requiredRequirements: stringArray,
  ...
  required: ["roleTitle", "readiness", "summary", "requiredRequirements", "niceToHaveRequirements", "demonstrated", "unclear", "missing", "improvements", "nextStep"],
...
export const analysisInstructions = `You are Career Pilot Job Fit Check. Compare a résumé with a job description using only the supplied text.
...
- Separate required requirements from preferred or nice-to-have requirements.
- "demonstrated" means the résumé contains direct evidence. Quote or closely paraphrase that evidence.
- "unclear" means potentially relevant evidence exists but is vague, indirect, or missing context.
...
- Return exactly three specific improvements, ordered by value.
