...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (rawPastedResume.length > MAX_TEXT_CHARS || rawJobDescription.length > MAX_TEXT_CHARS) {
    ...
    if (jobDescription.length < MIN_JOB_DESCRIPTION_CHARS) return NextResponse.json({ error: "Add a short summary of the role, including its main responsibilities or required skills." }, { status: 400 });
    ...
    runId = await startRun(sessionId, fingerprint(request), resumeMethod, resumeText.length, jobDescription.length);
    ...
    const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(model)}:generateContent`, {
      ...
      body: JSON.stringify({
        ...
        contents: [{ role: "user", parts: [{ text: `RESUME DATA:\n<resume>\n${redactContactDetails(resumeText)}\n</resume>\n\nJOB DESCRIPTION DATA:\n<job_description>\n${jobDescription}\n</job_description>` }] }],
        generationConfig: {
    ...
    const result = JSON.parse(outputText) as JobFitResult;
