...
export async function POST(request: Request) {
  try {
    const { name, email, sessionId, attribution = {}, offerToken, promoCode } = await request.json();
    ...
    const order = await createOrder(name.trim(), email.trim().toLowerCase(), offerToken, promoCode);
    ...
    await bestEffort(async () => {
      ...
      await supabaseRequest("checkout_attempts", {
        ...
        promo_code: order.offer === "FB299" ? "FB299" : null,
